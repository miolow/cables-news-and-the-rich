...
################################################################################
# all files 
all_06 = readtext("C:/Users/Michelle/Desktop/ContentAnalysis/Data/ByYear/2006/*.docx")

## Create columns for date, year, and day for year datafile
all_06 <- all_06 %>%
  add_column(date = substr(all_06$text, 1, 10)) %>%
  add_column(year = substr(all_06$text, 7, 10)) %>%
  add_column(day = substr(all_06$text, 11, 14)) 
# View(MSNBC_06)

## Tokenize
all_06_tk <- all_06 %>% unnest_tokens("word", text)

## Remove whitespaces
all_06_tk$word <- gsub("\\s+","", all_06_tk$word)
...
## Remove numbers
all_06_tk <- all_06_tk[-grep("\\b\\d+\\b", all_06_tk$word),]
...
highComp_match = str_detect(all_06_tk$word, 
...
all06_hiComp = sum(highComp_match, na.rm = TRUE)

# Count total number of words in 2006
ttlwrd_all_2006 = length(all_06_tk$word)

# Calculate percentage of HC words
perHC_all_2006 = (all06_hiComp/ttlwrd_all_2006)*100
...
highComp_sub = str_subset(all_06_tk$word, 
...
hiComppercent = (hiComp.per$n)/ttlwrd_all_2006
rm(hiComp.per)
...
ggplot(hiComp.per, aes(x = reorder(word, -percentage), y = percentage, fill = word))+
  ...
  xlab("")+
  ggtitle("Fox News & MSNBC 2006 - High Competence")+
...
# paste takes the dictionary and combines them into one cell
lowComp_match = str_detect(all_06_tk$word, pattern = paste(lowComp_dict, collapse = "|"))


# Count number of TRUE (in other words, number of matches)
sum(lowComp_match, na.rm = TRUE)

all06_loComp = sum(lowComp_match, na.rm = TRUE)

# Calculate percentage of HC words
perLC_all_2006 = (all06_loComp/ttlwrd_all_2006)*100

# Return items that matched
lowComp_sub = str_subset(all_06_tk$word, pattern = paste(lowComp_dict, collapse = "|"))
lowComp_sub
# View(lowComp_sub)
...
loComppercent = (loComp.per$n)/ttlwrd_all_2006
rm(loComp.per)
...
ggplot(loComp.per, aes(x = reorder(word, -percentage), y = percentage, fill = word))+
  ...
  ylab("Frequency in Percentage")+
  xlab("")+
  ggtitle("Fox News & MSNBC 2006 - Low Competence")+
  theme(plot.title = element_text(hjust = 0.5))+
  guides(fill=FALSE)
...
# str_detect detects patterns in tokenized file from high comp dictionary
# paste takes the dictionary and combines them into one cell
highMoral_match = str_detect(all_06_tk$word, 
...
all06_hiMoral = sum(highMoral_match, na.rm = TRUE)

# Calculate percentage of HC words
perHM_all_2006 = (all06_hiMoral/ttlwrd_all_2006)*100

# Return items that matched
highMoral_sub = str_subset(all_06_tk$word, 
...
hiMoralpercent = (hiMoral.per$n)/ttlwrd_all_2006
rm(hiMoral.per)
...
ggplot(hiMoral.per, aes(x = reorder(word, -percentage), y = percentage, fill = word))+
  ...
  ylab("Frequency in Percentage")+
  xlab("")+
  ggtitle("Fox News & MSNBC 2006 - High Morality")+
  theme(plot.title = element_text(hjust = 0.5))+
  guides(fill=FALSE)
...
# str_detect detects patterns in tokenized file from high comp dictionary
# paste takes the dictionary and combines them into one cell
lowMoral_match = str_detect(all_06_tk$word, 
                            pattern = paste(lowMoral_dict, collapse = "|"))

# Count number of TRUE (in other words, number of matches)
sum(lowMoral_match, na.rm = TRUE)

all06_loMoral = sum(lowMoral_match, na.rm = TRUE)

# Calculate percentage of LM words
perLM_all_2006 = (all06_loMoral/ttlwrd_all_2006)*100

# Return items that matched
lowMoral_sub = str_subset(all_06_tk$word, pattern = paste(lowMoral_dict, collapse = "|"))
lowMoral_sub
...
loMoralpercent = (loMoral.per$n)/ttlwrd_all_2006
rm(loMoral.per)
...
ggplot(loMoral.per, aes(x = reorder(word, -percentage), y = percentage, fill = word))+
  ...
  ylab("Frequency in Percentage")+
  xlab("")+
  ggtitle("Fox News & MSNBC 2006 - Low Morality")+
  theme(plot.title = element_text(hjust = 0.5))+
  guides(fill=FALSE)


## Adding 2006 values to master datafile
all06_rw <- c(2006, perHC_all_2006, perLC_all_2006, 
                perHM_all_2006, perLM_all_2006)

masterALL <- data.frame(2006, perHC_all_2006, perLC_all_2006, 
                     perHM_all_2006, perLM_all_2006)
colnames(masterALL) <- c("Year", "HiComp", "LowComp", "HiMoral", "LowMoral")

all.df <- data.frame(2006, perHC_all_2006, perLC_all_2006, 
                       perHM_all_2006, perLM_all_2006)
colnames(all.df) <- c("Year", "HiComp", "LowComp", "HiMoral", "LowMoral")

## Saving top words
highComp_06 <- highComp_sub
lowComp_06 <- lowComp_sub 
highMoral_06 <- highMoral_sub 
lowMoral_06 <- lowMoral_sub
